...
export default async function DashboardPage() {
  ...
  return (
    <div className="space-y-16">
      {/* ============================================================
          HERO — design HardwarePC : titre énorme, lède, CTA pill
          ============================================================ */}
      <section className="pt-2">
        <div className="flex items-center justify-between gap-3 mb-10 text-[11px] uppercase tracking-[0.14em] text-muted">
          <span>{today}</span>
      ...
      <section>
        <div className="flex items-end justify-between gap-3 mb-6">
          ...
          <span className="text-[10px] uppercase tracking-[0.14em] text-faint">01 · Profil</span>
        </div>
      ...
      <section>
        <div className="text-center max-w-xl mx-auto mb-10">
          <span className="badge-accent">Modules interactifs</span>
        ...
        <div className="grid sm:grid-cols-2 lg:grid-cols-3 gap-4">
          {MODULES.slice(0, 6).map((m, i) => (
            ...
            >
              ...
              <div>
                <h3 className="font-display text-lg font-semibold tracking-tight text-text">{m.title}</h3>
                <p className="text-[12px] uppercase tracking-[0.08em] text-muted mt-1">{m.tag}</p>
              </div>
            </Link>
          ))}
        </div>
      ...
      <section>
        <div className="flex items-end justify-between gap-3 mb-6">
          ...
          <span className="text-[10px] uppercase tracking-[0.14em] text-faint">02 · Progression</span>
        </div>
      ...
      <section className="grid lg:grid-cols-2 gap-6">
        <article className="card p-6">
          <div className="flex items-center justify-between mb-4">
            ...
            <span className="text-[10px] uppercase tracking-[0.14em] text-faint">03</span>
          </div>
          {nextLesson ? (
...
function QuickLink({ href, icon: Icon, title, desc, num }: { href: string; icon: React.ComponentType<{ className?: string }>; title: string; desc: string; num: string }) {
  return (
    <Link href={href} className="card card-hover p-5 flex items-center gap-4 group">
      <span className="text-[10px] uppercase tracking-[0.14em] text-faint w-6">{num}</span>
